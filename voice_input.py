...
# Try to import ElevenLabs for speech-to-text
try:
    from elevenlabs_speech import ElevenLabsSpeechController
...
class ElevenLabsDestinationListener(DestinationListener):
    """ElevenLabs-based speech-to-text for destination capture."""
    
    def __init__(self):
        if not ELEVENLABS_STT_AVAILABLE:
            raise VoiceInputError("ElevenLabs not available for speech-to-text")
        
        self.speech_controller = ElevenLabsSpeechController()
        self.speech_controller.start()
    
    def capture(self, config: DestinationCaptureConfig) -> str:
        """Capture destination using ElevenLabs speech-to-text."""
        try:
            result = self.speech_controller.recognize_speech(
                timeout=config.timeout_s,
                language=config.locale.split('_')[0]  # Extract language code
            )
            ...
            raise VoiceInputError(f"ElevenLabs STT failed: {e}") from e


class ElevenLabsCodewordSessionListener(TranscriptSessionListener):
    """ElevenLabs-based speech-to-text with codeword detection."""
    ...
    def capture_transcript(self, config: DestinationCaptureConfig) -> str:
        """Capture transcript with codeword using ElevenLabs speech-to-text."""
        try:
            result = self.speech_controller.recognize_speech(
                timeout=config.timeout_s,
                language=config.locale.split('_')[0]  # Extract language code
...
def capture_destination_by_voice(
    ...
    # Use ElevenLabs by default, fallback to macOS if not available
    if listener is None:
        if ELEVENLABS_STT_AVAILABLE and not cfg.prefer_on_device:
            backend = ElevenLabsDestinationListener()
        else:
            backend = MacOSSpeechDestinationListener()
    else:
        backend = listener
...
def capture_destination_with_codeword(
    ...
    if listener is None:
        if ELEVENLABS_STT_AVAILABLE and not cfg.prefer_on_device:
            backend = ElevenLabsCodewordSessionListener()
        else:
            backend = MacOSCodewordSessionListener()
    else:
        backend = listener

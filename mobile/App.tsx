...
export default function App(): React.JSX.Element {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const destination = await runVoiceDestinationOnboarding(appendLog);
  ...
  return (
    <SafeAreaView style={styles.root}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.title}>Assistive Nav Prototype</Text>
        <Text style={styles.subtitle}>
          Live step progression, GPS testing, speech feedback, and native vision warnings ({hasNativeVision ? "native ready" : "mock mode"}).
        </Text>

        <View style={styles.card}>
          <Text style={styles.label}>Navigation status</Text>
          <Text style={styles.small}>Destination: {confirmedDestination ?? "not set yet"}</Text>
        ...
        <View style={styles.card}>
          ...
          {!cameraPermission?.granted ? (
            <View style={styles.rowButtons}>
              ...
              />
              <Text style={styles.small}>Needed for on-device obstacle preview.</Text>
            </View>
          ) : (
            <>
              <View style={styles.rowButtons}>
                ...
                />
                <Text style={styles.small}>{cameraEnabled ? "Camera active" : "Camera hidden"}</Text>
              </View>
              {cameraEnabled ? <CameraView facing="back" style={styles.cameraPreview} /> : null}
            </>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  cameraPreview: { height: 220, borderRadius: 10, overflow: "hidden", marginTop: 8 },

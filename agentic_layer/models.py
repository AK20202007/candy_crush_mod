...
@dataclass
class SurfaceObservation(Serializable):
    kind: SurfaceKind
    confidence: float
    direction: Direction = Direction.UNKNOWN
    near_field_ratio: float = 0.0
    distance_m: Optional[float] = None
    source: str = "surface-perception"
    attributes: Dict[str, Any] = field(default_factory=dict)

    def __post_init__(self) -> None:
        self.kind = _enum(SurfaceKind, self.kind, SurfaceKind.UNKNOWN)
        self.confidence = _bounded_float(self.confidence, 0.0, 0.0, 1.0)
        self.direction = _enum(Direction, self.direction, Direction.UNKNOWN)
        self.near_field_ratio = _bounded_float(self.near_field_ratio, 0.0, 0.0, 1.0)
        if self.distance_m is not None:
            self.distance_m = max(0.0, float(self.distance_m))

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SurfaceObservation":
        return cls(
            kind=_enum(SurfaceKind, data.get("kind", "unknown"), SurfaceKind.UNKNOWN),
            confidence=float(data.get("confidence", 0.0)),
            direction=_enum(Direction, data.get("direction", "unknown"), Direction.UNKNOWN),
            near_field_ratio=float(data.get("near_field_ratio", 0.0) or 0.0),
            distance_m=data.get("distance_m"),
            source=str(data.get("source", "surface-perception")),
            attributes=dict(data.get("attributes", {})),
        )
...
class FrameContext(Serializable):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FrameContext":
        return cls(
            ...
            surfaces=[SurfaceObservation.from_dict(s) for s in data.get("surfaces", [])],

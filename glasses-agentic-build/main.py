...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--auto-location",
        action="store_true",
        help="Use macOS Location Services for the route origin when --origin is not set.",
    )
    parser.add_argument(
        "--location-timeout",
        type=float,
        default=8.0,
        help="Seconds to wait for macOS Location Services when --auto-location is used.",
    )
    parser.add_argument("--no-speech", action="store_true", help="Print agent decisions only; do not use system speech.")
    parser.add_argument(
        "--speech-provider",
        choices=["system", "elevenlabs"],
        default="system",
        help="Speech output provider.",
    )
    parser.add_argument("--elevenlabs-voice-id", default="JBFqnCBsd6RMkjVDRZzb", help="ElevenLabs voice ID for TTS.")
    parser.add_argument("--elevenlabs-tts-model", default="eleven_flash_v2_5", help="ElevenLabs TTS model ID.")
    parser.add_argument("--voice-destination", action="store_true", help="Record destination from microphone using ElevenLabs STT.")
    parser.add_argument("--voice-record-seconds", type=float, default=5.0, help="Seconds to record for --voice-destination.")
    parser.add_argument("--elevenlabs-stt-model", default="scribe_v2", help="ElevenLabs STT model ID.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Minimum repeat interval for repeated urgent safety speech.",
    )
    return parser
...
def main() -> None:
    ...
    speech = SpeechController(
        ...
        provider=args.speech_provider,
        elevenlabs_voice_id=args.elevenlabs_voice_id,
        elevenlabs_model_id=args.elevenlabs_tts_model,
    )
    speech.start()

    if args.voice_destination:
        try:
            destination = transcribe_from_microphone(
                duration_s=args.voice_record_seconds,
                model_id=args.elevenlabs_stt_model,
            )
            print(f"[speech->text] Destination: {destination!r}")
        except Exception as exc:
            print(f"[speech->text] Failed ({exc}); falling back to terminal input.")
    ...
    location_fix: Optional[LocationFix] = None
    origin_text = args.origin
    if args.auto_location and not origin_text:
        try:
            print("[main] Requesting current location from macOS Location Services...")
            location_fix = get_current_location(timeout_s=args.location_timeout)
            origin_text = f"{location_fix.longitude},{location_fix.latitude}"
            accuracy = ""
            if location_fix.horizontal_accuracy_m is not None:
                accuracy = f" accuracy=~{location_fix.horizontal_accuracy_m:.0f}m"
            print(
                f"[main] Current location: lat={location_fix.latitude:.6f} "
                f"lon={location_fix.longitude:.6f}{accuracy}"
            )
        except Exception as exc:
            print(f"[main] Auto-location unavailable ({exc}); using mock navigation.")

    if origin_text:
        if not ors_key:
            print("[main] Origin is set but OPENROUTESERVICE_API_KEY is unset; using mock navigation.")
        else:
            try:
                start = _parse_origin_lon_lat(origin_text)
    ...
    router = AgenticNavigationRouter(
        min_repeat_interval_ms=args.agent_repeat_ms,
        min_urgent_repeat_interval_ms=args.agent_urgent_repeat_ms,
    )
